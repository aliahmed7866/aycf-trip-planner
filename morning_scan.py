...
HTTP 400/error.availability or redirect back to the wallet for a listed route
on a particular day; both are normal zero-flight results. Authentication
redirects/interstitial HTML are never treated as zero availability.
"""
...
def _looks_like_login_html(response: requests.Response) -> bool:
    ...
    if _is_auth_location(final_url):
        return True
        ...
        for attempt in range(retries + 1):
            ...
            try:
                ...
                raise WizzIntegrationChanged(
                    ...
                    f"(HTTP {response.status_code}, {content_type}, final URL {response.url or self.dynamic_url}). "
...
def run(force: bool = False) -> dict:
    ...
    print(f"[AYCF] PDF {generated.isoformat()} | {len(route_pairs)} routes | {departure_start.date()}..{departure_end.date()} | station aliases {len(client.station_ids)} (+{added_aliases} fallback)", flush=True)
