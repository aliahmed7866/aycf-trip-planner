...
Android Chrome, validates every PDF station before polling, checks the route
network across its advertised date window, and persists positive and zero-flight
results in SQLite.

The PDF is route-level, not a per-day timetable. Multipass may therefore return
HTTP 400/error.availability or redirect back to the wallet for a listed route on
a particular day; both are normal zero-flight results. Authentication redirects
and unexpected HTML are never treated as zero availability.
"""
...
def _is_no_availability_400(response: requests.Response) -> bool:
    return (
        ...
        and _safe_wizz_error(response).strip().casefold().strip('"') == "error.availability"
    )
...
def _is_wallet_location(location: str) -> bool:
    low = str(location or "").casefold()
    return (
        "multipass.wizzair.com" in low
        and "/subscriptions/spa/private-page/wallets" in low
    )
...
def _looks_like_login_html(response: requests.Response) -> bool:
    ...
    markers = (
        "openid-connect/auth", "keycloak", 'name="password"', "name='password'",
...
class CapturedRequestWizzClient(WizzAYCFClient):
        ...
        for attempt in range(retries + 1):
            ...
            if 300 <= response.status_code < 400:
                ...
                if _is_auth_location(location):
                    raise WizzSessionExpired(
                        "Wizz redirected AYCF polling to authentication. Reconnect Wizz; "
                        "completed route checks remain cached."
                    )
                if allow_no_availability and _is_wallet_location(location):
                    ...
                    return None
                raise WizzIntegrationChanged(
                    f"Wizz redirected {context} with HTTP {response.status_code} "
                    f"to an unexpected location: {location or '<missing>'}"
                )
            ...
            try:
                ...
                if _looks_like_login_html(response):
                    raise WizzSessionExpired(
                        "Wizz returned its login/auth page during AYCF polling. Reconnect Wizz; "
                        "completed route checks remain cached."
                    ) from exc
                if attempt < retries:
                    self.html_retries += 1
                ...
                raise WizzIntegrationChanged(
                    ...
                    f"(HTTP {response.status_code}, {content_type}, final URL "
                    f"{response.url or self.dynamic_url}). Completed checks remain cached."
                ) from exc
    ...
    def preflight(self) -> dict:
        ...
        data = self._send_and_decode(
            template,
            "captured AYCF preflight",
            allow_no_availability=True,
        )
    ...
    def check(self, origin, destination, day):
        ...
        context = (
            f"AYCF search {origin} ({origin_id}) -> {destination} ({destination_id}) "
            f"on {day.isoformat()}"
        )
...
def _mirror_for_web(cache_root: str, df, generated) -> None:
    ...
    (Path(cache_root) / "last_update.txt").write_text(
        str(int(time.time())),
        encoding="utf-8",
    )
...
def run(force: bool = False) -> dict:
    ...
    station_names = sorted(
        set(df["departure_from"].astype(str).str.strip())
        | set(df["departure_to"].astype(str).str.strip())
    )
    ...
    if current and current.get("run_id") == run_id and current.get("scanned_at") and not force:
        return {
            "ok": True,
            "skipped": True,
            "reason": "PDF publication already scanned",
            "pdf_run_id": run_id,
        }
    if db.scan_in_progress(run_id) and not force:
        return {
            "ok": True,
            "skipped": True,
            "reason": "A scan for this PDF is already running",
            "pdf_run_id": run_id,
        }
    ...
    if not state:
        raise RuntimeError(
            "No saved Wizz session. Import a Wizz session before the scheduled scan."
        )
    ...
    print(
        ...
        f"{departure_start.date()}..{departure_end.date()} | "
        f"stations {station_report['resolved']}/{station_report['required']} resolved | "
        f"aliases {station_report['aliases']} | "
        f"normalized +{station_report['normalized_added']} | public-map +{station_report['public_added']}",
        flush=True,
    )
    if station_report["unresolved"]:
        unresolved = ", ".join(station_report["unresolved"])
        raise RuntimeError(
            "Station preflight failed before live scanning. Unresolved PDF stations: "
            + unresolved
        )
    print("[AYCF] Station preflight OK: every PDF city has an IATA mapping.", flush=True)
    ...
    try:
            ...
            for origin, destination in edges_by_day[day]:
                ...
                if processed == 1 or processed % progress_every == 0 or processed == total_checks:
                    ...
                    print(
                        ...
                        f"live {route_day_checks} | resumed {resumed_checks} | "
                        f"flights {flights_found} | no-availability {client.no_availability_responses} | "
                        f"wallet-redirects {client.wallet_redirects} | {rate:.2f} checks/s",
                        flush=True,
                    )
        ...
        return {
            ...
            "stations": station_report["required"],
...
if __name__ == "__main__":
    print(
        json.dumps(
            run(
                force=os.environ.get("AYCF_FORCE_MORNING_SCAN", "false").lower()
                == "true"
            ),
            indent=2,
        )
