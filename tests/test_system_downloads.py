...
def test_browser_download_links_and_live_log_refresh(monkeypatch, tmp_path):
    ...
    try:
        with playwright.sync_playwright() as runtime:
            ...
            page.reload()
            fresh_button = page.get_by_role('button', name='Clear pending & resume')
            ...
            with page.expect_navigation():
                fresh_button.click()
            assert commands[0][0][1][-1] == 'pending'
            assert commands[0][0][2] == 'manual-morning.log'
            assert commands[0][1]['allow_local_reset']
            page.get_by_text('Full rescan (repeat verified checks)', exact=True).click()
            page.get_by_role('button', name='Start full rescan', exact=True).click()
            assert len(commands) == 1  # Required confirmation blocks submission.
            page.get_by_role('checkbox', name='Recheck successful results too').check()
            with page.expect_navigation():
                page.get_by_role('button', name='Start full rescan', exact=True).click()
            assert len(commands) == 2 and commands[1][0][1][-1] == 'fresh'
            assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
            assert not errors
            browser.close()
    finally:
