...
class ScanScopeTests(unittest.TestCase):
    ...
    def test_scope_fingerprint_is_order_independent(self):
        a = {
            "origins": ["Liverpool", "Birmingham"],
            "destination_mode": "only",
            "destinations": ["Warsaw", "Budapest"],
        }
        b = {
            "origins": ["Birmingham", "Liverpool"],
            "destination_mode": "only",
            "destinations": ["Budapest", "Warsaw"],
        }
    ...
    def test_filter_routes_supports_all_only_and_exclude(self):
        ...
        self.assertEqual(
            filter_routes(pairs, {**base, "destination_mode": "all"}),
            [("Birmingham", "Warsaw"), ("Liverpool", "Budapest"), ("Liverpool", "Warsaw")],
        )
        self.assertEqual(
            filter_routes(pairs, {**base, "destination_mode": "only", "destinations": ["Warsaw"]}),
            [("Birmingham", "Warsaw"), ("Liverpool", "Warsaw")],
        )
        self.assertEqual(
            filter_routes(pairs, {**base, "destination_mode": "exclude", "destinations": ["Warsaw"]}),
            [("Liverpool", "Budapest")],
        )
