            ...
            for f in rows:
                db.execute(
                    """INSERT OR REPLACE INTO route_flights
                       ...
                       VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                    (pdf_run_id, origin, destination, day, f.flight_code, f.departure.isoformat(), f.arrival.isoformat(), f.departure_text, f.arrival_text, f.duration, now),
                )
            db.execute(
...
def cached_scan_itineraries(graph, db: ScanCacheDB, origin: str, destination: Optional[str], start_day: date, days: int = 4, max_stops: int = 1, min_transfer_minutes: int = 150, limit: int = 100, max_paths_per_day: int = 250) -> Tuple[List[Dict[str, Any]], int]:
    client = CachedFlightClient(db)
