...
from scan_scope import (AIRPORT_GROUPS, clean_exclusions, load_scope, normalize_name,
                        save_scope, scan_plan, endpoint_matches, endpoint_excluded)
...
def exclusion_catalog(pairs, scope):
    ...
    for name in sorted(names):
        grouped[country_for(name)].append({"name": name, "code": airport_code(name),
            "excluded": any(endpoint_matches(name, item) and normalize_name(name) == normalize_name(item)
                            for item in scope.get("excluded_airports") or [])})
    ...
    for a, b in pairs:
        for first in AIRPORT_GROUPS.get(normalize_name(a), [a]):
            for second in AIRPORT_GROUPS.get(normalize_name(b), [b]):
                if first != second:
                    route_pairs.add(tuple(sorted((first, second), key=normalize_name)))
    route_pairs.update(tuple(pair) for pair in clean_exclusions(scope)["excluded_routes"])
    selected = {tuple(pair) for pair in clean_exclusions(scope)["excluded_routes"]}
...
def exclusion_preview(pairs, scope):
    plan = scan_plan(pairs, scope)
    ...
    baseline = scan_plan(pairs, unrestricted)
    return {key: plan[key] for key in ("route_count", "checks", "request_units", "estimated_minutes")} | {
        ...
        "blocked_preferences": [name for name in scope.get("preferred_destinations") or [] if endpoint_excluded(name, scope)],
    }
...
def create_scan_settings_blueprint(route_catalog, csrf_ok):
    ...
    def page():
        ...
        return render_template("scan_exclusions.html", catalog=catalog, scope=scope,
                               preview=exclusion_preview(pairs, scope))
    ...
    def preview():
        ...
        return jsonify(exclusion_preview(pairs, scope))
